...
def test_basic_game():
    """Test that the game runs and produces reasonable output"""
    print("=== Testing Basic Game ===")
    game = TessellateGame()
    
    # Make a few moves
    test_moves = [
        (0, 0),  # RED plays top-left
        (0, 2),  # BLUE plays 
        (2, 0),  # RED plays
        (2, 2),  # BLUE plays
    ]
    
    for i, (r, c) in enumerate(test_moves):
        print(f"\nMove {i+1}: {'RED' if game.current_turn == TileState.RED else 'BLUE'} plays ({r}, {c})")
        success = game.make_move(r, c)
        print(f"Move successful: {success}")
        print(game.render_ascii())
        print(f"Valid moves remaining: {len(game.get_valid_moves())}")
...
if __name__ == "__main__":
    ...
    print("1. Do the blocked positions (x) make sense?")
    print("   - When we play at (0,0), should (0,1) and (1,0) be blocked?")
    print("2. Are the scores updating correctly?")
    print("   - Each player should start with score 1")
    print("   - Scores should multiply by island sizes")
    print("3. Does the turn switching work?")
    
    print("\n=== Please verify against JavaScript ===")
    print("Try these same moves in the browser game and check:")
    print("- Do the blocking patterns match?")
    print("- Do the scores match?")
    print("- Are the same positions playable?")